...
export class DetailsComponent implements OnInit {
  ...
  updateprice(){
    ...
    console.log('15s',this.respprice);
    if (this.change>=0){
		this.color="green";
      this.updown=true;
    }
  ...
  oninput(){
	  ...
	  this.totalprice=this.boughtnum*this.lastprice;
	  this.totalprice=this.totalprice.toFixed(2);
  }
  
  	clickBuy(){
	  this.boughtalert=true;
	  ...
	  console.log('Get portfolio from localstorage:',data);
		...
		if (data!=null){
			let boughtinstance=data.filter(s => s.ticker === this.symbol);
			if (boughtinstance.length > 0) {
				...
				data=data.filter(s=>s.ticker!==this.symbol);
				// add it back
				data.push({"ticker": this.symbol, "companyname": this.companyname, "quantity": this.curnum, "avgcost": this.curavgcost, "totcost": this.curtotalprice});
			}
			else{
				console.log('Did not buy this stock before', this.symbol);
				data.push({"ticker": this.symbol, "companyname": this.companyname, "quantity": this.boughtnum, "avgcost": this.lastprice, "totcost": this.totalprice});
			
			}
		}
		else{ // no data in bought
			data=[];
			data.push({"ticker": this.symbol, "companyname": this.companyname, "quantity": this.boughtnum, "avgcost": this.lastprice, "totcost": this.totalprice});
		}
	...
	clearlocalstorage(){
		...
		let data1 = JSON.parse(localStorage.getItem("storetickers"));
		console.log('Clear storetickers from localstorage:',data1);
		let data2 = JSON.parse(localStorage.getItem("boughttickers"));
		console.log('Clear boughttickers from localstorage:',data2);
	}
